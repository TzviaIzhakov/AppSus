...
import { revsService } from './reviews.service.js';
...
function query(filterBy = {}) {
  return storageService.query(BOOK_KEY).then((books) => {
    ...
    if (filterBy.minAmount) {
      books = books.filter(
        (book) => book.listPrice.amount >= filterBy.minAmount
      );
    }
...
// function addReview(bookId, review) {
//   // get(bookId).then((book) => console.log(book));
//   return get(bookId)
//     .then((book) => {
//       const newRev = { ...review, id: utilService.makeId() };
//       book.reviews.push(newRev);
//       revsService.createRevs(newRev);
//       return book;
//     })
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    ...
    // );
    // books.push(
    //   _createBook('at viverra venenatis', {
    //     amount: 108,
    //     currencyCode: 'ILS',
    //     isOnSale: false,
    //   })
    // );
    books = books.map((book) => ({ ...book, reviews: [] }));
